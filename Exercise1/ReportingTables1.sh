...
hive -e "CREATE  TABLE temp_practical_exercise_1.updates as select user_id, count(*) cnt FROM hive_practical_exercise_1.activitylog WHERE type='UPDATE' group by user_id ;"
...
hive -e "CREATE  TABLE temp_practical_exercise_1.inserts as SELECT user_id, count(*) InsertCount FROM hive_practical_exercise_1.activitylog WHERE type='INSERT' group by user_id ;"
if [ $? -eq 0 ]; then
...
hive -e "CREATE  TABLE temp_practical_exercise_1.deletes as SELECT user_id, count(*) deletecount FROM hive_practical_exercise_1.activitylog WHERE type='DELETE' group by user_id ;"
if [ $? -eq 0 ]; then
...
hive -e "CREATE  TABLE temp_practical_exercise_1.isactive as SELECT user_id, if  ( unix_timestamp() - ts < 172800, 1,0) s from temp_practical_exercise_1.activitytype ;"
...
hive -e "CREATE  TABLE temp_practical_exercise_1.uploadcount as  select user_id, count(*)upc from hive_practical_exercise_1.csv group by user_id  ;"
...
hive -e "DROP TABLE temp_practical_exercise_1.updates"
hive -e "DROP TABLE temp_practical_exercise_1.inserts"
hive -e "DROP TABLE temp_practical_exercise_1.deletes"
hive -e "DROP TABLE temp_practical_exercise_1.activitytype"
hive -e "DROP TABLE temp_practical_exercise_1.isactive"
hive -e "DROP TABLE temp_practical_exercise_1.uploadcount"
